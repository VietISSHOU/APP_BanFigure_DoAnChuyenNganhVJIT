    ...
    if(!empty($iddonhang)){
        ...
        foreach ($chitiet as $key => $value) {
            $truyvan = 'INSERT INTO `chitietdonhang`(`iddonhang`, `idsp`, `soluong`, `gia`)  
                            VALUES ('.$iddonhang["iddonhang"].','.$value["ipsp"].','.$value["soluong"].',"'.$value["giasp"].'")';  
            $data = mysqli_query($conn, $truyvan);   
            //xu li tru so luong trong kho            
            $truyvankho = 'SELECT `sltonkho` FROM `sanphammoi` WHERE `id`='.$value["ipsp"];  
            $data1 = mysqli_query($conn, $truyvankho);      
            $sltrenkho = mysqli_fetch_assoc($data1);

            $truyvankho2 = 'UPDATE `sanphammoi` SET `sltonkho`='.$sltrenkho["sltonkho"]-$value["soluong"].' WHERE `id`='.$value["ipsp"];  
